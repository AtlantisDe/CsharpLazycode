...
namespace CsharpLazycode.Module.WindowsService
{
    /// <summary>
    ...
    public class WindowsServiceContainer
    {
        ...
        public WindowsServiceContainer(string serviceName, string description, string[] args)
        {
            _serviceName = serviceName;
            _args = args;
    ...
    public class Uitil
    {
        public static bool IsServiceExisted(string _serviceName)
        {
            return ServiceController.GetServices().ToList().Exists(x => x.ServiceName == _serviceName);
        }
        ...
        public static string Servicestatesstring(string _serviceName)
        {
            ...
            try
            {
                var sever = ServiceController.GetServices().ToList().Find(x => x.ServiceName == _serviceName);

                switch (sever.Status)
                {
                    case ServiceControllerStatus.ContinuePending: state = "服务即将继续({0})"; break;
                    case ServiceControllerStatus.Paused: state = "服务已暂停({0})"; break;
                    case ServiceControllerStatus.PausePending: state = "服务即将暂停({0})"; break;
                    case ServiceControllerStatus.Running: state = "服务正在运行({0})"; break;
                    case ServiceControllerStatus.StartPending: state = "服务正在启动({0})"; break;
                    case ServiceControllerStatus.Stopped: state = "服务未运行({0})"; break;
                    case ServiceControllerStatus.StopPending: state = "服务正在停止({0})"; break;
                }

                state = string.Format(state, sever.Status);
            ...
            catch (Exception ex)
            {
                ...
                state = string.Format("异常[{0}]:{1} 核心服务可能未成功的安装.", System.Reflection.MethodBase.GetCurrentMethod().Name, ex.Message);
            }
            return state;
        }
    }
